...
sweep_result["outliers [%]"] = 100.0 * (
    sweep_result["n_outliers"] / sweep_result["n_tracks"]
)
sweep_result["failures [%]"] = (
    100.0 * (n_particles - sweep_result["n_tracks"]) / n_particles
)
...
sweep_components = (
    sweep_result[
        ...
        & (sweep_result["final_reduction_method"] == "maxWeight")
...
fig3, ax = make_coordinates_plot(
    "final_reduction_method", sweep_red_meth, figsize=(10, 3), cmap=cmap
)
