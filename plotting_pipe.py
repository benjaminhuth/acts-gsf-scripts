    ...
    def parse(self, line):
        if line.count("FinalParticle") == 1 and not line.count("End") == 1:
            ...
            if self.found_one_electron:
                print("Warning: found more than one electrons, expect exactely one electron")
                return True
...
class CovarianceProcessor(BaseProcessor):
    def __init__(self):
        self.gsf_recorder = CovarianceRecorder(enabler = "Gsf step", disabler = "KalmanFitter step")
        self.kf_recorder = CovarianceRecorder(enabler = "KalmanFitter step", disabler = "Gsf step")
    ...
    def parse_line(self, line):
        if line.count("Gsf step") == 1:
            ...
            if len(self.gsf_current_step_cmp_momenta) > 0:
                self.gsf_cmp_data.append((self.gsf_current_step_cmp_momenta, self.gsf_current_step_cmp_weights))
                self.gsf_current_step_cmp_momenta = []
                self.gsf_current_step_cmp_weights = []
...
class KalmanMomentumRecorder:
    ...
    def parse_line(self, line):
        if line.count("KalmanFitter step at") == 1 and not self.kalman_started_smoothing:
...
class MomentumGraph(BaseProcessor):
    ...
    def plot_components(self, ax, pathlengths, cmp_data, colors):
        for pl, (momenta, weights), color in zip(pathlengths, cmp_data, cycle(colors)):
            ...
            ax.scatter(pl * np.ones(len(momenta)), momenta, c=color, s=8, label='_nolegend_')
            
    def plot(self, ax, name, gsf_pathlengths, gsf_momenta, gsf_cmp_data, kalman_pathlengths, kalman_momenta):
        ...
        if len(gsf_pathlengths) > 0:
            ax.plot(gsf_pathlengths, gsf_momenta, linewidth=3, c='blue', label="GSF {}".format(name))
            ax.scatter(gsf_pathlengths, gsf_momenta, linewidth=3, c='blue', label="GSF {}".format(name))
            
        if len(kalman_pathlengths) > 0:
            ax.plot(kalman_pathlengths, kalman_momenta, linewidth=3, c='orange', label="Kalman {}".format(name))
            ax.scatter(kalman_pathlengths, kalman_momenta, linewidth=3, c='orange', label="Kalman {}".format(name))
    ...
    def process_data(self):
        ...
        print("self.gsf_forward_momentum_recorder.gsf_pathlengths", len(self.gsf_forward_momentum_recorder.gsf_pathlengths))
        print("self.kalman_momentum_recorder.kalman_pathlengths", len(self.kalman_momentum_recorder.kalman_pathlengths))
        
        # Forward
        ax = self.plot(ax, "forward",
        ...
        last_pathlengths = [ pl + self.gsf_backward_momentum_recorder.gsf_pathlengths[-1] for pl in self.gsf_last_part_momentum_recorder.gsf_pathlengths ]

        # Shorten if necessary
        if len(self.gsf_last_part_momentum_recorder.gsf_momenta) == len(last_pathlengths)+1:
            self.gsf_last_part_momentum_recorder.gsf_momenta = self.gsf_last_part_momentum_recorder.gsf_momenta[:-1]
        ...
        self.gsf_backward_momentum_recorder.gsf_momenta += self.gsf_last_part_momentum_recorder.gsf_momenta
        self.gsf_backward_momentum_recorder.gsf_cmp_data += self.gsf_last_part_momentum_recorder.gsf_cmp_data
        ...
        gsf_bwd_pls = mirror_pathlengths(np.array(self.gsf_backward_momentum_recorder.gsf_pathlengths))


        if len(self.kalman_momentum_recorder.kalman_pathlengths_backward) > 0:
            kalman_bwd_pls = mirror_pathlengths(np.array(self.kalman_momentum_recorder.kalman_pathlengths_backward))
        else:
            kalman_bwd_pls = []

        ax = self.plot(ax, "backward",
                       gsf_bwd_pls,
        ...
        if len(self.true_particle_recorder.pathlengths) > 0:
            ax.plot(self.true_particle_recorder.pathlengths,
                    self.true_particle_recorder.momenta,
                    linewidth=3, c='darkred', label="True momentum")
        ...
        ax.axvline(x=0, ymin=1.e-3, ymax=ax.get_ylim()[1], c='black')
...
class KalmanPlotter(BaseProcessor):
    ...
    def parse_line(self, line):
        ...
        if line.count("KalmanFitter step at") == 1:
            ...
            self.current_position_list.append(np.array([ float(split[-9]), float(split[-8]), float(split[-7])]))
...
class AverageTrackPlotter(BaseProcessor):
    ...
    def parse_line(self, line):
        if self.true_particle_recorder.parse(line):
            ...
            self.current_positions.append(np.array([ float(item) for item in line.split() ]))

    def process_data(self):
        fig, axes = plt.subplots(1,len(self.view_drawers))
        
        fig.suptitle("GSF average track (forward: {} steps, backward: {} steps, last: {} steps)".format(
            len(self.fwd_positions),
            len(self.bwd_positions),
            len(self.last_positions),
        ))

        positions = [self.fwd_positions, self.bwd_positions, self.last_positions]
...
class ComponentsPlotter(BaseProcessor):
    ...
    def process_data(self):
        ...
        for target_surface, direction, abs_step, component_positions in self.stages:
            ...
            fig.suptitle("Stepping {} towards {} ({} steps, starting from {})".format(direction,
                                                                                      target_surface,
...
class CsvCartesianDrawer:
    ...
    def draw_detector_impl(self, ax):
        if not self.simple_telescope:
            ax.scatter(self.points[:,0], self.points[:,1], color="grey")
        else:
...
class CsvXYDrawer(XYDrawer):
    def __init__(self, detector_csv, assume_telescope=False):
        super().__init__()
        if assume_telescope:
            self.drawer = CsvCartesianDrawer(detector_csv, ["cx", "cy"], ["bound_param1","bound_param3"])
        else:
...
class CsvXZDrawer(XZDrawer):
    def __init__(self, detector_csv, assume_telescope=False):
        super().__init__()
        if assume_telescope:
            self.drawer = CsvCartesianDrawer(detector_csv, ["cx", "cz"], ["bound_param0","bound_param2"])
        else:
...
class CsvZRDrawer(ZRDrawer):
    ...
    def draw_detector_impl(self, ax):
        ax.scatter(self.z, self.r, color="grey")
...
class LdmxXYDrawer(XYDrawer):
    ...
    def draw_detector_impl(self, ax):
        ...
        for x, y in zip(self.x, self.y):
            ax.plot(3*[x], [y+self.half_length, y, y-self.half_length], color="grey")
...
class LdmxXZDrawer(XZDrawer):
    ...
    def draw_detector_impl(self, ax):
        ...
        for x, z in zip(self.x, self.z):
            ax.plot(3*[x], [z+self.half_length, z, z-self.half_length], color="grey")
...
if __name__ == "__main__":
    input_processors = {
        ...
        "avgplot-steps": lambda drawers: AverageTrackPlotter(drawers, annotate_steps=True),
        ...
        "momentumgraph-opacity": lambda drawers: MomentumGraph(True, annotate_steps=True),
    ...
    parser.add_argument("processors", choices=list(input_processors.keys()) + ["all"], help="which processors geometry to use", nargs='+')
    parser.add_argument("--geometry-csv", default="detectors.csv")
    parser.add_argument("--geometry-type", choices=["csv", "csv-telescope", "ldmx"], default="csv")
    ...
    if args["geometry_type"] == "csv":
        ...
        detector_file = pathlib.Path(args["geometry_csv"])
        assert detector_file.exists()
        drawers = [ CsvXYDrawer(detector_file, assume_telescope=True), CsvXZDrawer(detector_file, assume_telescope=True) ]
